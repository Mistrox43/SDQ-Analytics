...
const OverviewTab: React.FC<OverviewTabProps> = ({ filteredData }) => {
  ...
  const projectTypeDistribution = useMemo(() => 
    calculateProjectTypeDistribution(filteredData), 
    [filteredData]
  );
  
  const lobDistribution = useMemo(() => 
    calculateLOBDistribution(filteredData), 
    [filteredData]
  );

  // Calculate status counts only once
  const statusCounts = useMemo(() => {
    return {
      investigation: filteredData.filter(d => d['Project Status'] === '01 - Investigation').length,
      planning: filteredData.filter(d => d['Project Status'] === '03 - Planning').length,
      inProgress: filteredData.filter(d => d['Project Status'] === '04 - In Progress').length,
      postGoLive: filteredData.filter(d => d['Project Status'] === '05 - Post Go-Live').length,
      complete: filteredData.filter(d => d['Project Status'] === '06 - Complete').length,
      pitTeam: filteredData.filter(d => d['Project Status'] === '00 - PIT Team Planning/Scheduling Activity').length,
      facilityLed: filteredData.filter(d => d['Project Status'] === '07 - Facility Led Project - Tracking Purpose Only').length,
      onHold: filteredData.filter(d => d['Project Status'] === '08 - On Hold').length,
    };
  }, [filteredData]);
              ...
              <div className="w-full bg-gray-200 h-2 rounded-full">
                <div className="relative">
                  ...
                  <div className="absolute left-0 -mt-5 flex flex-col items-center">
                    ...
                    <div className="text-xs mt-1 font-bold">{statusCounts.investigation}</div>
                  </div>
                  ...
                  <div className="absolute left-1/4 -mt-5 flex flex-col items-center">
                    ...
                    <div className="text-xs mt-1 font-bold">{statusCounts.planning}</div>
                  </div>
                  ...
                  <div className="absolute left-2/4 -mt-5 flex flex-col items-center">
                    ...
                    <div className="text-xs mt-1 font-bold">{statusCounts.inProgress}</div>
                  </div>
                  ...
                  <div className="absolute left-3/4 -mt-5 flex flex-col items-center">
                    ...
                    <div className="text-xs mt-1 font-bold">{statusCounts.postGoLive}</div>
                  </div>
                  ...
                  <div className="absolute right-0 -mt-5 flex flex-col items-center">
                    ...
                    <div className="text-xs mt-1 font-bold">{statusCounts.complete}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="mt-12">
            ...
            <div className="flex flex-nowrap space-x-4">
              <div className="bg-gray-100 p-4 rounded-lg flex-1">
                ...
                <div className="text-lg font-bold">{statusCounts.pitTeam}</div>
              </div>
              <div className="bg-gray-100 p-4 rounded-lg flex-1">
                <div className="text-xs text-gray-700">Facility Led Project</div>
                <div className="text-lg font-bold">{statusCounts.facilityLed}</div>
              </div>
              <div className="bg-gray-100 p-4 rounded-lg flex-1">
                <div className="text-xs text-gray-700">On Hold</div>
                <div className="text-lg font-bold">{statusCounts.onHold}</div>
              </div>
            </div>
          </div>
        </div>
